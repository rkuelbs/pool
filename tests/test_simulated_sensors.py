...
def make_plant(
    *,
    ...
    return SimulatedPlant(
        ...
        noise_stddev_scale=noise_stddev_scale,
    )
...
@pytest.mark.asyncio
async def test_pressure_sensor_rises_when_pump_runs_high_speed() -> None:
    plant = make_plant()
    sensor = sensor_by_id(
        build_default_simulated_sensors(plant),
        SensorId.PUMP_OUTPUT_PSI,
...
@pytest.mark.asyncio
async def test_chemical_sensors_publish_raw_readings() -> None:
    plant = make_plant()
    sensors = build_default_simulated_sensors(plant)
...
@pytest.mark.asyncio
async def test_chlorine_dosing_raises_orp_and_lowers_tank_level() -> None:
    plant = make_plant()
    sensors = build_default_simulated_sensors(plant)

    raw_orp_sensor = sensor_by_id(sensors, SensorId.RAW_ORP)
    raw_ph_sensor = sensor_by_id(sensors, SensorId.RAW_PH)
    tank_level_sensor = sensor_by_id(sensors, SensorId.TANK_LEVEL)

    initial_orp = await raw_orp_sensor.read()
    initial_ph = await raw_ph_sensor.read()
    ...
    dosed_ph = await raw_ph_sensor.read()
    dosed_tank_level = await tank_level_sensor.read()

    assert initial_orp.value == 675.0
    assert dosed_orp.value > initial_orp.value
    assert dosed_ph.value > initial_ph.value
    assert initial_tank_level.value == 100.0
    assert dosed_tank_level.value == 96.0
    assert plant.true_fc_ppm > 4.0


@pytest.mark.asyncio
async def test_simulated_noise_is_reproducible_with_seed() -> None:
    first = make_plant(noise_seed=123, noise_stddev_scale=1.0)
    second = make_plant(noise_seed=123, noise_stddev_scale=1.0)

    first_values = [first.pressure_psi("pump_output_psi") for _ in range(5)]
    second_values = [second.pressure_psi("pump_output_psi") for _ in range(5)]
